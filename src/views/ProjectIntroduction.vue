<template>
  <div class="w-full bg-zinc-100">
    ...
    <!-- 方向介绍正文 -->
    <div
      class="w-4/5 p-4 mt-8 transition-all duration-300 bg-white hover:scale-[0.98] lg:ml-52 myShadow"
    ...
    <!-- Q & A -->
    <div
      class="w-4/5 p-4 mt-8 transition-all bg-white hover:scale-[0.98] duration-300 lg:ml-52 myShadow"
      id="Q&A"
    >
      <h1 class="text-2xl font-bold tracking-wider text-sky-400">🎯 Q & A</h1>
      <div class="p-2 mt-2 border-t">
        <div v-for="(item, index) in FAQ" :key="index" class="pb-2">
          <p class="font-bold">
            <span class="pr-3 font-normal text-sky-400">{{
              `${index + 1}.`
            }}</span
            >{{ item?.question }}
          </p>
          <p class="indent-4">{{ item?.answer }}</p>
        </div>
      </div>
    </div>
...
const projectInfo = [
  ...
  },
  {
    ...
    leader: {
      ...
      grade: "2021级系统与技术方向",
      experience:
        "于2023年完成景点推荐平台的设计与实现，于2022年服务创新创业大赛中协助完成垃圾智能分类识别的项目。除此之外，在众多比赛中独立完成了 WEB 前端+ WEB 后端的开发任务，并在2022年中国高校计算机大赛-网络技术挑战赛中荣获国家级一等奖。",
      contact: "1370172202",
    },
    directionIntroduction: [
...
const FAQ = [
  ...
  },
  {
    question: "项目如何提交？",
    answer:
      "和光点计划要求一致，实现的代码通过 PR 的形式上传到 github 或国产开源平台，同时在学习过程中记录下自己的心得体会、实现思路和测试截图等，以项目笔记的方式上传。",
  },
];
</script>
