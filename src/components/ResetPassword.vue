<template>
  ...
  >
    <div class="px-5 py-4 bg-white rounded-md lg:w-1/4">
      <h1 class="text-xl font-bold tracking-widest text-sky-400">重置密码</h1>
      <!-- 密码 -->
...
const emit = defineEmits(["alert", "toggleShow"]);
...
async function handleReset() {
  try {
    ...
    let token = data?.data;
    // 重置用户密码
    await resetUserInfo(token, inputData.phoneNumber, inputData.password);
